...
const NewsPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const formattedPosts = data.posts.map((post) => ({
        ...
        profile: "./companyLogo.png",
        names: "Dynamic Company",
        date: new Date().toLocaleDateString(),
        linkedin: post.link,
      }));
      console.log("after fetched data from newsPage");
      // console.log(data)
      // console.log(formattedPosts)
      setPosts(formattedPosts);
      setLoading(false);
    };
    const fetchNewsnEvents = async () => {
      const res = await fetch("/api/newsnEvent/index");
      const data = await res.json();

      const formattedNewsnEvents = data.posts.map((post) => ({
        id: post._id,
        img: post.imageUrl,
        title: post.topic,
        content: post.content,
      ...
      setNewsnEvents(formattedNewsnEvents);
      setLoading(false);
    };
  ...
  useEffect(() => {
    console.log(selctedCardList);
    setSelctedCardList(newsnEvents);
  }, [newsnEvents]);
  // useEffect(() => {
  //   if (location.hash) {
      ...
      content: (
        <div style={{ width: "100vw" }}>
          ...
          <div className="blogs-cards-container">
            <a href={blogCardDetails[0].link} target="_blank">
              <div className="blog-card-1">
                <img
                  src={blogCardDetails[0].img}
                  alt="Blog"
                  className="blog-image-1"
                />
                <div className="blog-content">
                  <div className="blog-content-inside">
                    <h3 className="blog-title">{blogCardDetails[0].title}</h3>
                    <div className="blog-footer">
                      <div className="author-container">
                        <div className="img-container">
                          <img
                            src={blogCardDetails[0].profile}
                            alt="author"
                            className="author"
                          />
                        </div>
                        <div className="author-content">
                          <p className="author-name">
                            {blogCardDetails[0].names}
                          </p>
                          <div className="author-time">
                            <p className="blog-date">
                              {blogCardDetails[0].date}
                            </p>
                          </div>
                        </div>
                      </div>
                      <motion.svg
                        ...
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        {blogCardDetails[0].link.includes("linkedin")
                          ? linkedInSvg
                          : blogCardDetails[0].link.includes("instagram")
                          ? instagramSvg
                          : blogCardDetails[0].link.includes("youtub")
                          ? youtubeSvg
                          : twitterSvg}
                      </motion.svg>
                    </div>
                    <p className="blog-text">{blogCardDetails[0].content}</p>
                  </div>
                </div>
              </div>
            </a>
          ...
          <div className="newspagecardcontainer">
            <SwiperCarouselNews items={posts} />
        ...
        <div className="newspagecardcontainer" style={{ width: "100vw" }}>
          {visibleSwiper ? (
            <>
              ...
              >
                {newsnEvents.length > 0 ? (
                  newsnEvents.map((userdetails, idx) => {
                    return (
                  ...
                  <p>No News and Events available...</p>
                )}
              </Swiper>
              <div className="custom-pagination"></div>
            </>
